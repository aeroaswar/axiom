...
| 21 | The compound guide carries no price | The owner's call. The guide is education: the lots table shows dose, presentation and availability, the pathway cards show name, class and doses, and the price, the plan and the tax line live on the product page the guide links to. The gated wording that the guide still carried ("prices are shown after the acknowledgement · sign in") goes with it, since prices are open. The guide's basket link is named for the basket. | `compounds/[pathway]/page.tsx`, `compounds/[pathway]/[slug]/page.tsx` |
| 22 | No exclusive-of-tax notice on the storefront | The owner's call. The "excl. PPN" line goes from the shop cards, the card chooser, the product price block, the mini basket and the basket; the delivery note on the product page and the mini basket keeps what it says about delivery. PPN itself is unchanged: `site_settings.ppn_rate` still drives the quote, the invoice states it, and the confirmation still says the quote is itemised with PPN and delivery. | `shop-card.tsx`, `quick-add.tsx`, `mini-basket.tsx`, `request/page.tsx`, `products/[slug]/page.tsx` |
| 23 | The certificate reads as an A4 page | The sheet itself is the raised object: 794 x 1123 at 96 dpi, its own border and shadow, centred on the black ground. It was already A4, but the raised panel sat on the frame around it and the page inside was transparent, so the shape never read. The section keeps the page gutter (`padding-block`, not a shorthand that zeroes the sides), and on a phone the document's right-aligned cells wrap so the sheet never clips or scrolls sideways. The PDF is untouched: these rules are scoped to the page's frame. | `site.css` (`.coa-view`, `.coa-frame`) |
| 24 | The bottle leaves the catalogue; the storefront states no address | The owner's call. The steel water bottle is out of `supabase/seed.sql`, so a fresh database never creates it, and it is unpublished in the databases that already carry it — the stock ledger is append-only by rule, so a product with movements is withdrawn, never erased. Separately the storefront no longer says where AXIOM sits: the hero eyebrow, the footer line, the contact page's location row, the boilerplate, the meta descriptions, the dispatch copy, the Organization postal address and the certificate's issuer city all go. What stays is what a buyer or the law needs: the rupiah, Bahasa Indonesia, the Indonesian mobile format, the delivery service area, and the governing-law, tax and personal-data references on the legal pages. | `seed.sql`, `messages/*`, `site-footer.tsx`, `seo.ts`, `documents/coa.ts` |
| 25 | Merch takes photographs, the same way lots take pens | A product photograph is a square studio shot, so it gets its own path rather than the pen's 1672x940 one: `scripts/merch/build.py` reads one square source per slug from `scripts/merch/src`, writes `public/products/<slug>-{400,800,1280}.webp`, and lists the slug under `photos` in the shared manifest. `ProductImage` serves a photograph first, the rendered pen next, the row's own tile last, and the card, product figure and thumbnail each take a 1:1 shape when a photograph is what fills them. The source originals stay out of git; the built webp set ships. | `scripts/merch/build.py`, `product-image.tsx`, `site.css` |
