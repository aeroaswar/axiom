...
| 11 | CoA publication | The sample CoA row is seeded in **`seed_dev.sql` only**, so a production database built per this README (`seed.sql`, never `seed_dev.sql`) has no sample row and `/api/coa` answers "no sample certificate is published". The dev fixture points at `public/coa/sample-bpc-157-10mg.pdf`, which is not in the repository either. Publishing one means a `seed.sql` row and the file; `coa/` is the tool that produces it. | `coa_documents.is_sample`, `supabase/seed_dev.sql`, `coa/` |
