...
export type Line = { sku: string; qty: number; site_id: string | null };
...
export type PublicRequest = { name: string; clinic: string | null; role: string | null; email: string | null; whatsapp: string | null };
...
export type QuoteLine = { name: string; dose: string; qty: number };
...
export async function getQuoteLines(uid: string, number: string): Promise<QuoteLine[]> {
  ...
  return rows.map(r => ({ name: String(r.name), dose: String(r.dose), qty: Number(r.qty) }));
}
