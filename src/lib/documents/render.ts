...
// The same document template, the same CSS and the same headless print as src/lib/pdf.ts — but
// react-dom/server is imported at call time rather than at module scope, because the App Router
// refuses a static import of it anywhere in the server graph, route handlers included.
//
// LEAD: once src/lib/pdf.ts moves its own `renderToStaticMarkup` import inside documentHtml(),
// this file collapses to `export { documentPdf } from '@/lib/pdf'` and should be deleted.
