...
export type EventRow = {
  ...
  subject_type: 'quote' | 'order' | 'account' | 'variant' | 'subscription';
...
const KNOWN = new Set([
  ...
  'order_packing', 'order_dispatched', 'renewal_due', 'renewal_overdue', 'renewal_to_accept',
]);
...
const ICONS: Record<string, string> = {
  ...
  reorder_overdue: 'reorder', reorder_due: 'reorder', stockout: 'flask', renewal_due: 'reorder', renewal_overdue: 'reorder',
};
...
export async function EventFeed({ rows }: { rows: EventRow[] }) {
  ...
  const href = (e: EventRow) =>
    e.subject_type === 'account' ? `/console/clients/${e.subject_id}`
      : e.subject_type === 'subscription' ? '/console/subscriptions'
